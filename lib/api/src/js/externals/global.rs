...
impl Global {
    ...
    pub fn set(&self, ctx: &mut impl AsContextMut, val: Value) -> Result<(), RuntimeError> {
        ...
        let descriptor = js_sys::Object::new();
        let (type_str, new_value) = match val {
            Value::I32(i) => ("i32", JsValue::from_f64(i as _)),
            Value::I64(i) => ("i64", JsValue::from_f64(i as _)),
            Value::F32(f) => ("f32", JsValue::from_f64(f as _)),
            Value::F64(f) => ("f64", JsValue::from_f64(f)),
        ...
        js_sys::Reflect::set(
            &descriptor,
            &"mutable".into(),
            &global_ty.mutability.is_mutable().into(),
        )?;

        let js_global = JSGlobal::new(&descriptor, &new_value).unwrap();
        let global = VMGlobal::new(js_global, global_ty);
        unsafe {
            *self.handle.get_mut(ctx.as_context_mut().objects_mut()) = global;
        }
        Ok(())
    }

    pub(crate) fn from_vm_export(ctx: impl AsContextMut, vm_global: VMGlobal) -> Self {
        Self {
